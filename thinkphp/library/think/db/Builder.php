...
abstract class Builder
{
    ...
    // 数据库表达式
    protected $exp = ['eq' => '=', 'neq' => '<>', 'gt' => '>', 'egt' => '>=', 'lt' => '<', 'elt' => '<=', 'notlike' => 'NOT LIKE', 'not like' => 'NOT LIKE',
        'like' => 'LIKE', 'in' => 'IN', 'exp' => 'EXP', 'notin' => 'NOT IN', 'not in' => 'NOT IN', 'between' => 'BETWEEN', 'not between' => 'NOT BETWEEN', 'notbetween' => 'NOT BETWEEN',
        'exists' => 'EXISTS', 'notexists' => 'NOT EXISTS', 'not exists' => 'NOT EXISTS', 'null' => 'NULL', 'notnull' => 'NOT NULL', 'not null' => 'NOT NULL', '> time' => '> TIME',
        '< time' => '< TIME', '>= time' => '>= TIME', '<= time' => '<= TIME', 'between time' => 'BETWEEN TIME', 'not between time' => 'NOT BETWEEN TIME',
        'notbetween time' => 'NOT BETWEEN TIME','find_in_set'=>'FIND_IN_SET'];
...
//                $vs .=$v .';';
//            }
//            throw new Exception("where express error:" .$exp .'--' .$val[1] .'--' .$rule .'--' .$options .'--' .$binds .'--' .$bindName);
                // $value[0] 是要查找的值，$value[1] 是逗号分隔的字符串列表字段
            $whereStr .= ' FIND_IN_SET( ' . $this->parseValue($value, $field) . ', ' .$field .')';
        }
        return $whereStr;
    }
