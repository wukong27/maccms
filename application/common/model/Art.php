...
class Art extends Base {
    ...
    public function infoData($where,$field='*',$cache=0)
    {
        ...
        //单独取出 content
        $cont = model('ArtContent')->infoData(['id'=>['eq',$info['art_id']]]);
        if(!empty($cont)){
            $info['art_content']=$cont['art_content'];
            $info['art_page_list'] = mac_art_list($info['art_title'], $info['art_note'], $info['art_content']);
            $info['art_page_total'] = count($info['art_page_list']);
        }
    ...
    public function saveData($data)
    {
        ...
        if(false === $res){
            ...
            $data2 = [];
            $data2['id']=$data['art_id'];
            $data2['art_content'] = $content;
            model('ArtContent')->insert($data2,!empty($data['art_id']));
        }
